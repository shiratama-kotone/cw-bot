...
// レベルに応じた基本XP（Lv100→2、Lv500→3、Lv1000→5）
function baseXpForLevel(lv){
  if(lv>=1000) return 5;
  if(lv>=500)  return 3;
  if(lv>=100)  return 2;
  return 1;
  ...
  discordClient.on(Events.InteractionCreate, async(interaction)=>{
    ...
    try{
      ...
      if(cmd==='rank'){
        ...
        const addedRole=role?interaction.guild.roles.cache.get(role.roleId):null;
        await reply(null,{
