...
const {
  Client, GatewayIntentBits, Events, REST, Routes// Chatwork Bot for Render (WebHook版 - 全ルーム対応)
// server.js
...
if(DISCORD_BOT_TOKEN){
  ...
  discordClient.on(Events.InteractionCreate, async(interaction)=>{
    ...
    try{
      ...
      if(cmd==='money'){if(!interaction.guild){await replyErr('サーバー内でのみ使えるよ');return;}const eco=await getEconomy(interaction.guild.id,interaction.user.id);await reply(null,{title:`${interaction.member.displayName} の所持金`,fields:[{name:'財布',value:`**${fmt(eco.wallet)}**`,inline:true},{name:'銀行',value:`**${fmt(eco.bank)}**`,inline:true},{name:'合計',value:`**${fmt(parseInt(eco.wallet)+parseInt(eco.bank))}**`,inline:true},{name:'職業',value:eco.job,inline:true}],color:0xf39c12});return;}
      if(cmd==='money_send'){if(!interaction.guild){await replyErr('サーバー内でのみ使えるよ');return;}const target=interaction.options.getUser('user'),amount=interaction.options.getInteger('amount'),gid=interaction.guild.id;if(target.id===interaction.user.id){await replyErr('自分には送れないよ');return;}if(target.bot){await replyErr('botには送れないよ');return;}const myEco=await getEconomy(gid,interaction.user.id);if(parseInt(myEco.wallet)<amount){await replyErr(`財布の残高が足りないよ！財布: ${fmt(myEco.wallet)}`);return;}await dbQuery('UPDATE discord_economy SET wallet=wallet-$1 WHERE guild_id=$2 AND user_id=$3',[amount,gid,interaction.user.id]);await getEconomy(gid,target.id);await dbQuery('UPDATE discord_economy SET wallet=wallet+$1 WHERE guild_id=$2 AND user_id=$3',[amount,gid,target.id]);await reply(`<@${target.id}> に **${fmt(amount)}** 送ったよ！`,{title:'送金完了',color:0x2ecc71});return;}
      if(cmd==='bank'){if(!interaction.guild){await replyErr('サーバー内でのみ使えるよ');return;}const eco=await getEconomy(interaction.guild.id,interaction.user.id);await reply(null,{title:`${interaction.member.displayName} の銀行`,fields:[{name:'銀行残高',value:`**${fmt(eco.bank)}**`,inline:true},{name:'財布',value:`**${fmt(eco.wallet)}**`,inline:true}],color:0x3498db});return;}
      if(cmd==='bank_deposit'){if(!interaction.guild){await replyErr('サーバー内でのみ使えるよ');return;}const amount=interaction.options.getInteger('amount'),gid=interaction.guild.id,uid=interaction.user.id,eco=await getEconomy(gid,uid);if(parseInt(eco.wallet)<amount){await replyErr(`財布の残高が足りないよ！財布: ${fmt(eco.wallet)}`);return;}await dbQuery('UPDATE discord_economy SET wallet=wallet-$1,bank=bank+$1,updated_at=NOW() WHERE guild_id=$2 AND user_id=$3',[amount,gid,uid]);await reply(`**${fmt(amount)}** を銀行に預けたよ！`,{title:'預け入れ完了',color:0x3498db});return;}
